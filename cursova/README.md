...
- 3. Next on AWS Console I launched EC2 instance with the same Ubuntu Server, connected to it through SSH with Putty, created there the same bash script with nano text editor and executed it. So I got my app running on internet with true public IP (but only after manually opened the 80 HTTP port in Security Group settings from AWS Console :) )
